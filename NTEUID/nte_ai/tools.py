...
@ai_tools(
    covers=["异环账号主页/UID/资产概览图片面板"],
    category="common",
    context_tags=["异环"],
    capability_domain="异环面板",
    timeout=60.0,
)
...
@ai_tools(
    covers=["异环体力/树脂/精力/本性像素/都市活力实时面板"],
    category="common",
    context_tags=["异环"],
    capability_domain="异环面板",
    timeout=60.0,
...
@ai_tools(
    covers=["异环成就进度图"], category="common", context_tags=["异环"], capability_domain="异环面板", timeout=60.0
)
...
@ai_tools(
    covers=["异环角色攻略/养成图/配装推荐"],
    category="common",
    context_tags=["异环"],
    capability_domain="异环资料",
    timeout=60.0,
)
...
@ai_tools(
    covers=["异环抽卡记录/出金/保底统计图"],
    category="common",
    context_tags=["异环"],
    capability_domain="异环抽卡",
    timeout=60.0,
)
...
@ai_tools(
    covers=["异环已绑定账号列表"], category="common", context_tags=["异环"], capability_domain="异环账号", timeout=30.0
)
...
async def nte_switch(bot: Bot, ev: Event, target: str = "") -> str:
    """查询并切换当前活跃的异环账号。用户说"切换账号、换号、切号"时调用；target 填账号序号或 center_uid，留空则轮换到下一个账号。"""
    await switch_binding(bot, ev, target)
    return "已切换异环账号。"


@ai_tools(
    covers=["删除异环登录态/解绑当前或全部账号"],
    category="common",
    context_tags=["异环"],
    capability_domain="异环账号",
    timeout=30.0,
...
@ai_tools(
    covers=["异环插件功能帮助图"], category="common", context_tags=["异环"], capability_domain="异环日常", timeout=15.0
)
